...
struct Parallelogram updateBin(struct Bin r, int j, int diagonal, int tau, int w, int qGramLength, int refLength) { // function unused
...
bool updateBinAndCheckParallelogram(struct Bin * r, int j, int diagonal, int tau, int w, int qGramLength, int refLength) {
    if (j - w + qGramLength > r->max) {
        if (r->count >= tau) {
            return true;
        }
...
bool checkAndResetBinAndCheckParallelogram(struct Bin * r, int j, int diagonal, int tau, int w, int qGramLength, int refLength) {
    if (j - w + qGramLength > r->max) {
        if (r->count >= tau) {
            return true;
        }
...
//    printf("%i", 1 << 2*qGramLength);
    ...
    struct Bin * bins = (Bin *) calloc(binsLength, sizeof(struct Bin));
